...
class MailThread(models.AbstractModel):
    ...
    def message_process(self, model, message, custom_values=None,
        ...
        if self.env.context.get('params') or self.env.context.get('default_fetchmail_server_id') is not None:
            if self.env.context.get('params'):
                if self.env.context.get('params').get('model') == 'fetchmail.server':
                    ...
                    if emails_from_list:
                        fetch_list = [e.strip().lower() for e in emails_from_list.split(',') if e.strip()]
        ...
        raw_from = (msg_dict.get('email_from') or '').strip()
        match = re.search(r'<([^>]+)>', raw_from)
        email_from = match.group(1).strip().lower() if match else raw_from.lower()

        def matches_catch_list(email, catch_items):
            if not catch_items:
                return True
            for item in catch_items:
                ...
                if '@' in item and not item.startswith('@'):
                    ...
                    if email.endswith('@' + clean_item) or email.endswith('.' + clean_item) or email == clean_item:
                        return True
            return False

        if fetch_list and not matches_catch_list(email_from, fetch_list):
            _logger.info(
                'Mail from %s to %s with Message-Id %s is not in catch list %s; '
                'continuing with processing to ensure it reaches CRM.',
                msg_dict.get('email_from'), msg_dict.get('to'), msg_dict.get('message_id'), fetch_list
            )
        else:
            ...
            if email_from.endswith(('agoda.com', 'mailer.agoda.com', 'em.agoda.com', 'messaging.agoda.com')) or email_from in ('d365labs@gmail.com', 'sudarsanan1996@gmail.com'):
                ...
                if data.get('Booking ID'):
                    if len(self.env['crm.lead'].search([('booking_id', '=', data.get('Booking ID'))])) == 0:
                        ...
                        _logger.warning(
                            'Agoda booking %s already exists in CRM; skipping duplicate creation.',
                            data.get('Booking ID')
                        )
                        return
                else:
                    _logger.warning(
                        'Agoda email from %s matched, but no Booking ID could be extracted; '
                        'falling back to standard mail routing.',
                        email_from
                    )
            if email_from.endswith('airbnb.com') or email_from in ('d365labs@gmail.com', 'sudarsanan1996@gmail.com'):
                if 'reservation confirmed' in msg_dict.get('subject', '').lower():
                    ...
                    if len(self.env['crm.lead'].search([('booking_id', '=', data.get('confirmation_code'))])) == 0:
                        ...
                        return
            if email_from.endswith(('go-mmt.com', 'makemytrip.com', 'mmt.com')) or email_from in ('d365labs@gmail.com', 'sudarsanan1996@gmail.com'):
                data = {
                ...
                if data.get('Booking ID'):
                    if len(self.env['crm.lead'].search([('booking_id', '=', data.get('Booking ID'))])) == 0:
                        ...
                        _logger.warning(
                            'MakeMyTrip booking %s already exists in CRM; skipping duplicate creation.',
                            data.get('Booking ID')
                        )
                        return
                else:
                    _logger.warning(
                        'MakeMyTrip email from %s matched, but no Booking ID could be extracted; '
                        'falling back to standard mail routing.',
                        email_from
                    )
            if email_from.endswith('booking.com') or email_from in ('d365labs@gmail.com', 'sudarsanan1996@gmail.com'):
                ...
                booking_data = None
                # 2. Filter for booking.com URLs containing res_id or reservation_id
                for link in links:
                    ...
                    if "booking.com" in href_lower and ("res_id=" in href_lower or "reservation_id=" in href_lower):
                        # Extract booking ID from query params using regex
                        match = re.search(r"(?:res_id|reservation_id)=(\d+)", href, re.IGNORECASE)
                ...
                if not booking_data:
                    # Fallback: extract booking number from email body or subject
                    b_match = re.search(r"(?:booking(?:\s+reference|\s+number)?|res(?:ervation)?\s*(?:id|number)?)\s*[:#-]?\s*(\d{8,12})", cleaned_text, re.IGNORECASE)
                    if not b_match:
                        b_match = re.search(r"(\d{9,11})", msg_dict.get('subject', ''))
                    if b_match:
                ...
                if not booking_data:
                    ...
                    if len(self.env['crm.lead'].search([('booking_id', '=', booking_id)])) == 0:
                        ...
                        _logger.warning(
                            'Booking.com booking %s already exists in CRM; skipping duplicate creation.',
                            booking_id
                        )
                        return
        ...
        if not routes:
            # Guaranteed fallback: ensure every incoming message creates a CRM lead
            # so no email arriving in the mailbox silently disappears!
            lead_custom_values = dict(custom_values or {})
            lead_custom_values.setdefault('type', 'opportunity')
            routes = [('crm.lead', 0, lead_custom_values, None, None)]
            _logger.info(
                'No standard mail route found for message %s from %s to %s; '
                'routing directly to crm.lead fallback so it appears in CRM.',
                msg_dict.get('message_id'), msg_dict.get('email_from'), msg_dict.get('to')
            )
